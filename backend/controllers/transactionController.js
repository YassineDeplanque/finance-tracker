...
export const getIncomeMonth = async (req, res) => {
    try{
        const db = await connection();
        const query = "SELECT  * FROM income WHERE YEAR(date) = YEAR(CURDATE()) AND MONTH(date) = MONTH(CURDATE()) ORDER BY date desc";
        const [result] = await db.execute(query);
...
export const getIncomeThreeMonths = async (req, res) => {
    try{
        const db = await connection();
        const query = "SELECT  * FROM income WHERE DATE_FORMAT(date, '%Y-%m') >= DATE_FORMAT(CURDATE() - INTERVAL 2 MONTH, '%Y-%m') ORDER BY date desc";
        const [result] = await db.execute(query);
...
export const getSumIncomeThreeMonths = async (req, res) => {
    try{
        const db = await connection();
        const query = "SELECT  SUM(AMOUNT) AS total FROM income WHERE DATE_FORMAT(date, '%Y-%m') >= DATE_FORMAT(CURDATE() - INTERVAL 2 MONTH, '%Y-%m')";
        const [result] = await db.execute(query);
...
export const editExpenses = async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.execute(query, [amount, category, date, id]);
